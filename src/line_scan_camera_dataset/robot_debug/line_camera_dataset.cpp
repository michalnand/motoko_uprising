...
LineCameraDataset::LineCameraDataset()
{
...
void LineCameraDataset::update_sensors(GLGUI &gui, Json::Value &json)
{
    this->json_result.result["sensors"] = json;
...
void LineCameraDataset::update_camera(GLGUI &gui, Json::Value &json)
{
    Json::Value camera_pixels = json["line_camera_pixels"];

    this->json_result.result["line_camera_pixels"] = camera_pixels;


    gui.set_variable("line scan camera", camera_pixels);

    gui.main();
}

std::string LineCameraDataset::get_rounded(float value, unsigned int precision)
{
...
void LineCameraDataset::save_result(std::string file_name)
{
    json_result.save(file_name);
}
